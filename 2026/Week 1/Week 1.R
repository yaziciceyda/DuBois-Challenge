...
mexico_col[which(mexico$name %in% c("Coahuila",
                                    "Tamaulipas", "Veracruz",
                                    "Tabasco", "Campeche",
                                     "Quintana Roo"))] <- "#654321"
# For Nuevo León and Yucatán

mexico_col[which(mexico$adm1_code %in% c("MEX-2714", "MEX-2737"))] <- "#654321"


# Belize

belize <-  ne_states(iso_a2 = 'BZ', returnclass = "sf")
belize_col <- rep("#654321", length(belize$name))
...
honduras_col[which(honduras$name %in% c("Gracias a Dios"))] <- "#654321"


# For Cortés, Atlántida and Colón

honduras_col[which(honduras$adm1_code %in% c("HND-647", "HND-637",
                                             "HND-638"))] <- "#654321"
...
nigaragua_col[which(nigaragua$name %in% c("Rio San Juan"))] <- "#654321"

# For Atlántico Norte, Atlántico Sur

nigaragua_col[which(nigaragua$adm1_code %in% c("NIC-657", "NIC-644"))] <- "#654321"
...
costa_rica_col[which(costa_rica$adm1_code %in% c("CRI-1329"))] <- "#654321"
...
# For Ngöbe Buglé and Colón

panama_col[which(panama$adm1_code %in% c("PAN-3454",
                                         "PAN-1337"))] <- "#654321"
...
colombia_col[which(colombia$name %in% c("Sucre",
                                        "Magdalena",
                                        "La Guajira",
                                        "Cesar"))] <- "#654321"

# For Córdoba, Bolívar, Atlántico

colombia_col[which(colombia$adm1_code %in% c("COL-1316",
                                         "COL-1413", "COL-1412"))] <- "#654321"
...
# For Falcón, Mérida, Guárico, Anzoátegui, Bolívar

venezuela_col[which(venezuela$adm1_code %in% c("VEN-23", "VEN-27",
                                               "VEN-45", "VEN-40",
                                               "VEN-39", "VEN+99?"))] <- "#654321"
...
brazil_col[which(brazil$name %in% c("Roraima",
                                    "Mato Grosso",
                                    "Mato Grosso do Sul",
                                    "Santa Catarina",
                                    "Rio Grande do Sul"))] <- "#654321"

# For Amapá, Pará, São Paulo, Paraná

brazil_col[which(brazil$adm1_code %in% c("BRA-681", "BRA-594",
                                      "BRA-1311", "BRA-613"))] <- "#654321"



brazil_col[which(brazil$name %in% c("Rio Grande do Norte",
                                    "Pernambuco",
                                    "Alagoas", "Sergipe",
                                    "Tocantins", "Bahia",
                                    "Distrito Federal",
...
# For Maranhão, Piauí, Ceará, Goiás, Paraíba, Espírito Santo


brazil_col[which(brazil$adm1_code %in% c("BRA-593", "BRA-622", "BRA-626", "BRA-625",
                                         "BRA-621", "BRA-1294"))] <- "black"
...
final_plot <- ggdraw(p) +
  ...
  draw_label("THE BLACK PEOPLE IN GEORGIA.", x = 0.5, y = 0.99,
  ...
  draw_label("ROUTES OF THE AFRICAN SLAVE TRADE.",
             x = 0.64, y = 0.25,
             fontfamily = "ps", hjust = 0.5, size = 25) +
  geom_point(aes(x = 0.35, y = 0.2), colour = "black", size = 11) +
  geom_star(aes(x = 0.35, y = 0.2), fill = "ivory", size = 8) +
  draw_label("THE STATE OF GEORGIA.",
             x = 0.56, y = 0.2,
             fontfamily = "ps", hjust = 0.5, size = 25) +
  draw_label("THIS CASE IS DEVOTED TO A SERIES OF CHARTS , MAPS AND OTHER DEVI-\nCES DESIGNED TO ILLUSTRATE THE DEVELOPMENT OF\nTHE AMERICAN BLACK PEOPLE IN A SINGLE TYPICAL STATE OF THE UNITED STATES.",
             x = 0.50, y = 0.13,
             fontfamily = "ps", hjust = 0.5, size = 25) +
  draw_label("THE PROBLEM OF THE 20TH CENTURY IS THE PROBLEM OF THE\nCOLOR-LINE.",
             x = 0.56, y = 0.05,
             fontfamily = "ps", hjust = 0.5, size = 25) +
  draw_label("#DuboisChallenge26| Week 1 | Prepared by C. YAZICI",
...
ggsave("Week1.png", final_plot, width = 20, height = 22, dpi = 72)
